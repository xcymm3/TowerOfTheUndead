  ...
  methods: {
    update() {
      ...
      this.rows = AntimatterDimensions.all.map((dim, index) => {
        ...
        const count = this.mode === "one" ? 1 : Math.max(1, dim.howManyCanBuy);
        const cost = this.mode === "one" ? dim.cost : dim.cost.times(count);
        return { tier: dim.tier, name: window.UndeadTheme.armyNames[index],
          ...
          canBuy: dim.isAvailableForPurchase && dim.isAffordable && !capped && !this.continuum,
          ...
          unlock: dim.tier > DimBoost.totalBoosts + 4 ? "筑塔后解锁" : "需先召唤上阶军团"
        };
      });
    },
    choose(mode) {
  ...
  <section class="army-management">
    <div class="army-toolbar">
      ...
      <div class="buy-modes" role="group" aria-label="召唤数量">
        <button :class="{ selected: mode === 'one' }" @click="choose('one')">单次</button>
        <button :class="{ selected: mode === 'ten' }" @click="choose('ten')" title="补至下一个十次购买，资源不足时购买可负担数量">十次</button>
        <button :class="{ selected: mode === 'max' }" @click="choose('max')">最大</button>
      </div>
    ...
    <div class="army-rows">
      <article v-for="row in rows" :key="row.tier" class="army-row" :class="{ 'is-locked': !row.available }" :data-tier="row.tier">
        ...
        <div class="army-price"><b>{{ row.cost }}</b><small>{{ row.currency }}</small></div>
        <button :disabled="!row.canBuy" :data-buy-tier="row.tier" @click="buy(row.tier)"
          :title="row.capped ? '缚时领域购买上限' : !row.available ? row.unlock : '召唤' + row.name">
    ...
    <div v-if="sacrificeVisible" class="sacrifice-panel">
      ...
      <button :disabled="!canSacrifice" :title="sacrificeReason" @click="sacrifice">献祭</button>
    </div>
